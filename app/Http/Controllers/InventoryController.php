...
class InventoryController extends Controller
{
    ...
    {
        return view("inventory.positions", ['user' => Auth::user(), 'inventory' => Inventory::get()]);
    }
    
    public function local() 
    {
        $inventory = Inventory::where('description', 'Quartinho')->first();
        return view("inventory.local", ["user" => Auth::user(), 'inventory' =>$inventory]);
    }
    
    public function distribuitors() 
    {
        return view("inventory.distribuitors", [
            "user" => Auth::user(), 
    ...
    public function getAdvancedSearch() 
    {
        ...
        return view("inventory.advanced_search", ['user' => Auth::user()]);
    }
    
    public function postAdvancedSearch(Request $request) 
    {
        $result = array();
        $products = Product::where('sku', 'like', '%' . ($request->product_sku == null ? '---' : $request->product_sku) . '%')
                           ->get();
        
        foreach($products as $product) 
        {
            if ($request->input("delta_filter")) 
            {
                $inventories = Inventory::where('active', 's')->get();
                
                foreach ($inventories as $inventory) 
                {
                    $inv_product = InventoryProduct::where('inventory_id', $inventory->inventory_id)->where('product_id', $product->product_id)->orderBy("created_at")->first();
                    ...
                    if ($inv_product != null) 
                    {
                        ...
                        $item["description"] = $product->description_pt;
                        $item["quantity"] = $inv_product->quantity;
                        $item["inventory"] = $inventory->description;
                        $item["last_update"] = date_format(date_create($inv_product->created_at), "d/m/Y");
                        $item["sale_price"] = "-";
            ...
            if ($request->input("dist_filter")) 
            {
                $positions = DistribuitorInventoryProduct::where('product_id', $product->product_id)->orderBy("created_at", "desc")->get();
                $included = array();
                
                foreach ($positions as $pos) 
                {
                    $dist = Distribuitor::where('distribuitor_id', $pos->distribuitor_id)->first();
                    $in = false;
                    
                    foreach ($included as $id) 
                    {
                        if ($id == $dist->distribuitor_id) 
                        {
                            $in = true;
                        }
                    }    
                    ...
                    if ($pos != null && !$in) 
                    {
                        ...
                        $item["inventory"] = $dist->user()->name . " " . $dist->user()->surname;
                        $item["last_update"] = date_format(date_create($pos->created_at), "d/m/Y");

                        $aux = $dist->getProductSellPrice($product->product_id);

                        $item["sale_price"] = "R$ " . ($aux == null ? 0 : number_format($aux->value, 2, ".", ","));

                        array_push($result, $item);
                        array_push($included, $dist->distribuitor_id);
                    }
                }
            }
        }
    ...
    public function getByDistribuitor($dist_id) 
    {
        $dist = Distribuitor::where('distribuitor_id', $dist_id)->first();
        
        return $dist->getInventoryProducts();
    }
}