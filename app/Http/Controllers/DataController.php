...
class DataController extends Controller
{
    ...
    public function customers() 
    {
        ...
        return view('data.customers', ['customers' => $customers]);
    }
    
    public function products() 
    {
        $products = Product::where('active', 's')->whereIn('product_vrc_id', [1,3])->get();
        
        return view('data.products', ['products' => $products]);
    }
    ...
    public function getConsole() 
    {
        ...
        return view('data.console', ['user' => $user]);
    }
    ...
    public function runBatch(Request $request) 
    {
        ...
        {
            $batch = $request->file("batch_file");
            $date = strtotime(date_default_timezone_get());
            $path = storage_path("batches/" . date("m-Y", $date) . "/");
            
            $filename = "Batch_" . rand() . ".sql";
            
            $batch->move($path, $filename);
            
            $contents = file_get_contents($path . $filename);
            
            foreach (explode(";", $contents) as $cmd) 
            {
                try {
                    DB::statement($cmd);
    ...
    public function postConsole(Request $request) 
    {
        ...
        {
            $query = $request->input("command");
            
            foreach (explode(";", $query) as $cmd) 
            {
                try {
                    DB::statement($cmd);
                } catch (\Illuminate\Database\QueryException $e) {
                    echo "Erro no comando " . $cmd;
                }
            }
