...
class SubOrderController extends Controller
{
    ...
    public function detail($order_id) 
    {
        ...
        return view('sub_order.detail', ['order' => $order, 'user' => $user]);
    }
    
    
    public function showNotes($order_id) 
    {
        $notes = SubOrderNote::where('sub_order_id', $order_id)
                                 ->get();
        
        $order = SubOrder::where('sub_order_id', $order_id)->first();
        
        return view("sub_order.note", 
                    ["user" => Auth::user(), 
                     "notes" => $notes,
                     "order" => $order]);
    }
    
    public function newNote($order_id, Request $request) 
    {
        $note = new SubOrderNote;
        
        $note->sub_order_id = $order_id;
    ...
    public function reprove($order_id) 
    {
        $order = SubOrder::where('sub_order_id', $order_id)->first();
        
        $order->sub_order_status_id = 2;
        
        $order->save();
        
        $mngs = User::where('user_type', UserType::where('cod', 40)->first()->user_type_id)->get();
        
        DB::table('order_product')->where('order_id', $order->order_id)
                                  ->where('distribuitor_id', $order->distribuitor()->distribuitor_id)
                                  ->update(['dist_approval' => 'n']);
            
        foreach ($mngs as $m) {
            $note = new Notification;

            $note->from_user_id = $order->distribuitor()->user()->user_id;
            $note->to_user_id = $m->user_id;
            $note->subject_id = 7;
            $note->status_id = NotificationStatus::where('description', 'Enviado')->first()->notification_status_id;
            $note->text = "O Distribuidor " . $order->distribuitor()->user()->name . " " . $order->distribuitor()->user()->surname . " reprovou o pedido #" . $order->distribuitor()->delta_code . $order->order()->order_id;
    ...
    {
        $order = SubOrder::where('sub_order_id', $order_id)->first();
        return view('print.sub_order', ['order'=>$order]);
    }
    
    public function sendTo($sub_order_id, $email, Request $request) 
    {
        try {
            $order = SubOrder::where('sub_order_id', $sub_order_id)->first();
            $user = Auth::user();
            
           Mail::send('emails.sub_order_client', ['order' => $order], function ($m) use ($user, $email, $order) {
                $m->replyTo($user->email, $user->name . " " . $user->surname);
                $m->to($email, $name = null);
                $m->subject("Delta Faucet :: Pedido #" . $order->distribuitor()->delta_code . $order->order_id);
            });
    ...
    public function approve($order_id) 
    {
        ...
        foreach ($mngs as $m) {
            ...
            $note->subject_id = 7;
            $note->status_id = NotificationStatus::where('description', 'Enviado')->first()->notification_status_id;
            $note->text = "O Distribuidor " . $order->distribuitor()->user()->name . " " . $order->distribuitor()->user()->surname . " aprovou o pedido #" . $order->distribuitor()->delta_code . $order->order()->order_id;
