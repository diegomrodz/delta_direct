...
use App\Influencer;
use App\Jobs\SendWeeklyAgendaEmail;
use App\Jobs\SendMonthlyAgendaEmail;
...
class AgendaController extends Controller
{
    ...
    {
        $users = User::where('active', 's')->whereIn('user_type', [1, 2])->get();
        
        foreach ($users as $user) 
        {
            if ($user->user_id == 46) 
            {
                $this->dispatch(new SendMonthlyAgendaEmail($user));
            }
        }
    ...
    public function sendWeeklyEmails() 
    {
        ...
        foreach ($users as $user) 
        {
            if ($user->user_id == 46 || $user->getUserType()->cod == 20) 
            {
                $this->dispatch(new SendWeeklyAgendaEmail($user));
            }
        }
