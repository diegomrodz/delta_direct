...
class InboxController extends Controller
{
    ...
    public function index() 
    {
        ...
        return view('inbox.index', ['user' => $user]);
    }
    ...
    public function newPost() 
    {
        ...
        return view('inbox.new', ['user' => $user]);
    }
    ...
    public function createDraft(Request $request) 
    {
        ...
        foreach ($request->input("ccs") as $cc) 
        {
            ...
            $s = new PostUserStatus;
            
            $s->post_id = $post->post_id;
            $s->user_id = $cc;
    ...
    public function starIt($post_id) 
    {
        $user = Auth::user();
        $post = PostUserStatus::where('post_id', $post_id)->where('user_id', $user->user_id)
                               ->update(['important'=> 's']);
    }
    
    public function removeStar($post_id) 
    {
        $user = Auth::user();
        $post = PostUserStatus::where('post_id', $post_id)->where('user_id', $user->user_id)
                              ->update(['important'=> 'n']);
    ...
    public function deletePostFrom($post_id, $user_id) 
    {
        $post = PostUserStatus::where('post_id', $post_id)->where('user_id', $user_id)
                              ->update(['active'=> 'n']);
    }
    
    public function reply($post_id, Request $request) 
    {
        $user = Auth::user();
        ...
        PostUserStatus::where('post_id', $post->post_id)->update([
            'has_been_seen' => 's',
            'active' => 's'    
        ]);
    }
    ...
    public function newFaq(Request $request) 
    {
        ...
        $mngs = User::where('user_type', 3)->where('active', 's')->get();
    ...
    public function detail($post_id) 
    {
        $post = Post::where('post_id', $post_id)->first();
        $user = Auth::user();
        $status = PostUserStatus::where('post_id', $post->post_id)
                                 ->where('user_id', $user->user_id)
                                 ->update(['has_been_seen'=> 's']);
        
        return view('inbox.detail', ['post' => $post, 'user' => $user]);
    }
