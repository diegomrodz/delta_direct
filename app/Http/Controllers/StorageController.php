...
class StorageController extends Controller
{
    ...
    {
        $user = Auth::user();
        return view('storage.index', ['user' => $user]);
    }
    ...
    public function upload(Request $request) 
    {
        ...
        $directory = CloudDirectory::where('cloud_directory_id', $request->input("type"))->first();
        
        $cfile = new CloudFile;
        
        $cfile->user_id = $user->user_id;
        $cfile->cloud_directory_id = $directory->cloud_directory_id;
        $cfile->active = 'n';
        ...
        {
            $cfile->filename = $cfile->cloud_file_id . "_" . $user->distribuitor()->delta_code . "_" . $directory->name . "_" . date_format(date_create($cfile->created_at), "m_Y");
        }
        
        $filename = $cfile->filename . "." . $file->getClientOriginalExtension();
        $path = storage_path("/files/" . $directory->dir_path . "/");
        
        $file->move($path, $filename);
        
        $cfile->filepath = $path . $filename;
