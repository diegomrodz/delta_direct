...
class ProfileController extends Controller
{
    ...
    public function detail($user_id) 
    {
        ...
        return view('profile.detail', ['user' => Auth::user(), 'detail' => $user]);
    }
    
    public function leave_message($user_id, Request $request) 
    {
        $user = Auth::user();
    ...
    public function editUser($user_id, $prop, $value) 
    {
        User::where('user_id', $user_id)->update([$prop => $value]);
    }
    ...
    public function editRepresentantCompany($company_id, $prop, $value) 
    {
        RepresentantCompany::where('representant_company_id', $company_id)->update([$prop => $value]);
    }
    
    public function change_avatar($user_id, Request $request) 
    {
        $user = User::where('user_id', $user_id)->first();
        
        $avatar_file_att = $request->file("avatar_file_att");

        $filename = $user->user_id . "." . $avatar_file_att->getClientOriginalExtension();
        
        $path = base_path("/static_files/avatars/");

        $avatar_file_att->move($path, $filename);
        
        $user->avatar_path = "http://static.deltafaucetrep.com/avatars/" . $filename;
    ...
    public function changePassword(Request $request) 
    {
        ...
        {
            return abort(403, "Senhas não batem!");
        }
