...
class UserController extends Controller
{
    ...
    public function resetPassword(Request $request) 
    {
        ...
        Mail::send('emails.reset_password', ['user' => $user], function ($m) use ($user) {
            ...
            $m->subject("Delta Faucet :: Senha Resetada");
        });
    }

    /*
	 * Displays the register form if the method of submition is different from POST.
	 * 
	 * @param $request Request
	 * @return Response
     */
    public function register($ticket, Request $request) 
    {
        try {
            $ticketParts = explode("-", $ticket);

            $user = User::where("email", $ticketParts[0])->firstOrFail();

            if ( ! $request->isMethod('post'))
        	{
       			return view('auth.register', ["user" => $user, "ticket" => $ticket]);
        	}

        	$name = $request->input('register_name');
        	$surname = $request->input('register_surname');
        	$email = $request->input('register_email');
        	$password = $request->input('register_password');
        	$password2 = $request->input('register_password2');
            ...
            $usr_type = UserType::where('cod', $ticketParts[1])->first();

            $user->user_type = $usr_type->user_type_id;

            if ($usr_type->cod == 10) 
            {
                $cpf = $request->input('register_cpf');
                $tel = $request->input('register_tel');
                $cel = $request->input('register_cel');
