...
class Project extends Model
{
     ...
     * @var array
     */
    protected $fillable = ['representant_id',
                           'manager_id',
                           'project_type_id',
                           'project_stage_id',
                           'project_closed_status_id',
                           'distribuitor_id',
                           'influencer_id',
                           'organization_id',
                           'title',
	...
	{
		return Distribuitor::where('distribuitor_id', $this->distribuitor_id)->first();
	}
	
    public function type() 
    {
        return ProjectType::where('project_type_id', $this->project_type_id)->first();
    }
    
    public function stage() 
    {
        return ProjectStage::where('project_stage_id', $this->project_stage_id)->first();
    }
    
    public function notes() 
    {
        return ProjectNote::where('project_id', $this->project_id)->where('active', 's')->get();
    }
    
    public function closedStatus() 
    {
        return ProjectClosedStatus::where('project_closed_status_id', $this->project_closed_status_id)->first();
    }
    
	public function files() 
	{
		return CloudFile::where('cloud_directory_id', 4)->where('entity_id', $this->project_id)->where('active', 's')->get();
	}
	
    public function influencer() 
    {
        if ($this->influencer_id != null) 
        {
            return Influencer::where('influencer_id', $this->influencer_id)->first();
        } 
        else if ($this->organization_id != null) 
        {
            return Customer::where('customer_id', $this->organization_id)->first();
        }
        
        return null;
    }
	
	public function getAllClosedStatus() 
	{
		return ProjectClosedStatus::where('active', 's')->get();
	}
    
    public function influencers() 
    {
    	return ProjectInfluencer::where('project_id', $this->project_id)->where('active', 's')->orderBy("level", "desc")->get();
    }

	public function budgets() 
	{
		return ProjectBudget::where('project_id', $this->project_id)->where('active', 's')->get();
	}
	
	public function totalBudgetValue() 
	{
		return collect($this->budgets())->reduce(function ($carry, $item) {
			if ($item->flag_for_dp == 's') 
			{
				return $carry + $item->budget()->product_values;
			}
			return $carry;
		});
	}
