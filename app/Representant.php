...
class Representant extends Model
{
     ...
     * @var string
     */
    protected $primaryKey = 'representant_id';
     ...
     * @var array
     */
    protected $fillable = ['user_id', 'manager_id', 'representant_company_id', 'representant_profile_id', 'phone', 'cellphone', 'cpf'];
    ...
    {
        return RepresentantPortfolio::where('representant_id', $this->representant_id)
                                    ->where('active', 's')->get();
    }
    
    public function profile() 
    {
        return RepresentantProfile::where('representant_profile_id', $this->representant_profile_id)->first();
    }
    ...
    {
        return RepresentantCompany::where('representant_company_id', $this->representant_company_id)->first(); 
    }
    
    public function getBudgetDrafts() 
    {
        return RepresentantBudget::where('representant_id', $this->representant_id)
                                 ->where('active', 's')
                                 ->where('is_draft', 's')
                                 ->get();
    }
    ...
    {
        return Order::where('representant_id', $this->representant_id)
    ...
    {
        if ($this->company()->has_shared_portfolio == 's') 
        {
            $orders = array();
            
            foreach ($this->company()->representants() as $rep) 
            {
                foreach (Order::where('representant_id', $rep->representant_id)->where('active', 's')->get() as $order) 
                {
        ...
        {
            return Order::where('representant_id', $this->representant_id)->where('active', 's')->get();
        }
    }
    
	public function getProjectsToProspect() 
	{
		return Project::where('representant_id', $this->representant_id)
					  ->where('project_stage_id', 6)
					  ->where('active', 's')
					  ->get();
	}
	
    public function getEndedOrders() 
    {
        return Order::where('representant_id', $this->representant_id)
                    ->where('active', 's')
                    ->where(function ($query) {
