...
class Influencer extends Model
{
     ...
     * @var array
     */
    protected $fillable = ['influencer_type_id', 'representant_id', 'manager_id',
                          'distribuitor_id', 'customer_id', 'entity_derived_id', 'entity_derived_type',
                           'name', 'surname', 'email', 'website', 'phone', 'phone2', 'cellphone',
                           'cpf_cnpj', 'active', 'created_at', 'updated_at'];

    public function type() 
    {
        return InfluencerType::where('influencer_type_id', $this->influencer_type_id)->first();
    }
    
	public function projects() 
	{
		return ProjectInfluencer::where('active', 's')->where('influencer_id', $this->influencer_id)->get();
	}
	...
	{
		if ($this->entity_derived_id == null) 
		{
			return null;
		}
		
		if ($this->entity_derived_type == 'bc') 
		{
			return CustomerBusinessContact::where('customer_business_contact_id', $this->entity_derived_id)->first();
		}
		else 
		{
			return CustomerPartner::where('customer_partner_id', $this->entity_derived_id)->first();
		}
	}
    
    public function resp() 
    {
        if ($this->representant_id != null) 
        {
            return $this->representant();
        }
        elseif ($this->manager_id != null) 
        {
            return $this->manager();
        }
        elseif ($this->distribuitor_id != null) 
        {
