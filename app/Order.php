...
class Order extends Model
{
    ...
    protected $fillable = [
        ...
        'customer_id',
        'order_status_id',
        'representant_budget_id',
        'order_payment_condition_id',
        'payment_details',
        'observation',
        'final_observation',
        'manager_observation',
        ...
        public function getInterestRate() {
            $paymentCondition = $this->order_payment_condition_id;
    ...
    {
        return RepresentantBudget::where('representant_budget_id', $this->representant_budget_id)->first();
    }
    
    public function getProducts() 
    {
        return OrderProduct::where('order_id', $this->order_id)->where('active', 's')->get();
    }
    
    public function paymentCondition() 
    {
        return OrderPaymentCondition::where('order_payment_condition_id', $this->order_payment_condition_id)->first();
    }
    
    public function getStatus() 
    {
        return OrderStatus::where('order_status_id', $this->order_status_id)->first();
    }
    
    public function getSubOrders() 
    {
        return SubOrder::where('order_id', $this->order_id)->get();
    }
    
    public function allOrderMarginAnalysis() 
    {
        $totalMargin = 0;
        $pCount = 0;
        ...
        {
            if ($p->product()->margin_analysis != null) 
            {
                $pCount += 1;
                $totalMargin += $p->product()->margin_analysis;
            }
        }
        ...
        return $totalMargin / $pCount;
    }
    
    public function hasPendingProducts() 
    {
        if ($this->order_status_id == 5) {
            return count(OrderProduct::where('order_id', $this->order_id)->where('dist_approval', 'n')->get()) > 0;
        } else {
            return false;
        }
    }
