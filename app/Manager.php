...
class Manager extends Model
{
     ...
     * @var array
     */
    protected $fillable = ['user_id', 'phone', 'cellphone', 'cpf'];
    ...
    {
        return Order::where('manager_id', $this->manager_id)->where('order_status_id', 1)->where('active', 's')->get();
    }
    
    public function getInProgressOrders() 
    {
        return Order::where('manager_id', $this->manager_id)
                    ->where('active', 's')
    ...
    {
        return Order::where('manager_id', $this->manager_id)->where('active', 's')->get();
    }
    ...
    public function getPortfolio() 
    {
        ...
        $portfolio = ManagerPortfolio::where('manager_id', $this->manager_id)->where('active', 's')->get();
        $reps = $this->getRepresentants();
        $customers = Customer::where('active', 's')->get();
        
        
        foreach ($portfolio as $p) 
        {
            array_push($output, $p->getCustomer());
        }
        ...
        foreach ($customers as $c) 
        {
            ...
            if ($rep != null) 
            {
                foreach ($reps as $r) 
                {
                    if ($r->representant_id == $rep->representant_id) 
                    {
                        $is = true;
                        break;
                    }
    ...
    {
        return Representant::where('manager_id', $this->manager_id)->where('active', 's')->get();
    }
    
    public function getBudgets() 
    {
        return RepresentantBudget::where('manager_id', $this->manager_id)
