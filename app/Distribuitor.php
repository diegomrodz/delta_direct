...
class Distribuitor extends Model
{
     ...
     * @var string
     */
    protected $primaryKey = 'distribuitor_id';
    ...
    {
        return DistribuitorInventoryProduct::where('distribuitor_id', $this->distribuitor_id)
                                           ->where('distribuitor_inventory_product.active', 's')
                                           ->where('distribuitor_inventory_product.quantity', '>', 0)
                                           ->join('product', 'distribuitor_inventory_product.product_id', '=', 'product.product_id')
                                           ->get();
    }
    
    public function getPortfolio() 
    {
        $portfolio = DistribuitorPortfolio::where('distribuitor_id', $this->distribuitor_id)->get();
        
        return collect($portfolio)->map(function ($e) { return $e->getCustomer(); })->all();
    }
    ...
    public function user() 
    {
     ...
     {
        return RepresentantBudget::where('distribuitor_id', $this->distribuitor_id)
                                 ->where('active', 's')
                                 ->get();        
     }
    ...
    {
        $orders = Order::where('order.active', 's')
                       ->where('representant_budget.distribuitor_id', $this->distribuitor_id)
        ...
        return collect($orders)->map(function ($e) { return Order::where('order_id', $e->order_id)->first(); })->all();
    }
    
    public function getPendingOrdersApproval() 
    {
        return SubOrder::where('distribuitor_id', $this->distribuitor_id)->where('sub_order_status_id', 1)->where('active', 's')->get();
    }
    
    public function getPendingCreditAnalysis() 
    {
        return CustomerCreditAnalysisRequest::where('distribuitor_id', $this->distribuitor_id)
                                            ->where('customer_credit_analysis_request_status_id', 1)
                                            ->get();
    }
    
    public function getProductInventoryBalance($product_id) 
    {
        return DistribuitorInventoryProduct::where('distribuitor_id', $this->distribuitor_id)
                                            ->where('distribuitor_inventory_product.active', 's')
    ...
    public function getProductSellPrice($product_id) 
    {
        return ProductSellPrice::where('distribuitor_id', $this->distribuitor_id)
                                ->where('active', 's')
                                ->where('product_id', $product_id)
                                ->orderBy('updated_at', 'desc')
                                ->first();
