...
class Post extends Model
{
     ...
     * @var array
     */
    protected $fillable = ['user_id', 'tag_id', 'subject', 'text', 'has_been_seen', 'important','active', 'created_at', 'updated_at'];
    ...
    public function tag() 
    {
        return PostTag::where('post_tag_id', $this->tag_id)->first();
    }
    ...
    {
        return SubPost::where('post_id', $this->post_id)->where('active', 's')->get();
    }
    
    public function ccs() 
    {
        return PostCC::where('post_id', $this->post_id)->get();
    }
    
    public function isImportantTo($user) 
    {
        $status = PostUserStatus::where('post_id', $this->post_id)->where('user_id', $user)->first();
        
        return $status->important == 's';
    }
    
    public function hasBeenSeenBy($user) 
    {
        $status = PostUserStatus::where('post_id', $this->post_id)->where('user_id', $user)->first();
        
        return $status->has_been_seen == 's';
    }
